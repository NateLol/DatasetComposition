...
def parse_args():
    ...
    parser.add_argument('--stride', dest='stride', help='desired stride',
                        default=25, type=int)
    parser.add_argument('--resume', dest='resume', help='desired stride',
                        default=25, type=int)
...
if __name__ == '__main__':
    ...
    fps=0
    frames=0
    video = cv2.VideoCapture(os.path.join('./', args.name))
    if int(major_ver) < 3:
    ...
    while rval:
        ...
        if c % args.stride == 0:
            path = os.path.join(folder, '{:06d}'.format(c / args.stride) + '.jpg')
